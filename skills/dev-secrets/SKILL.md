...
This is a workflow-only skill. No CLI is installed by default.

The `dev-secrets` CLI is not implemented by this repository. Do not run or recommend `dev-secrets ...` unless the target project has installed a compatible implementation and `command -v dev-secrets` succeeds. When no compatible CLI is found, output the workflow as a plan with `Status: needs setup`. Never pretend commands ran.
...
2. This is a workflow-only skill. No CLI is installed by default.
3. Do not run `dev-secrets ...` unless the project has installed a compatible implementation and `command -v dev-secrets` succeeds.
4. You will not print, summarize, expose, or directly read real secret values.
5. Real values must be handled through local CLI or vault prompts, not chat.
6. Key names can reveal vendors, architecture, data categories, and internal systems. Key names may be shown only for manifest, classification, warning, or planning purposes.
7. If multiple env files exist, the user chooses which file to import.
8. Project-file edits are suggestions-only unless a dedicated local CLI command gets explicit approval.
9. Any child process receiving secrets can still leak through logs, crashes, telemetry, snapshots, or subprocesses.
10. If a value may have leaked, rotate it.
...
2. Check CLI availability with a non-secret lookup such as `command -v dev-secrets`; do not run `dev-secrets doctor`, `dev-secrets scan`, or any other `dev-secrets ...` command before that check succeeds.
3. Inspect only safe repo signals: tracked file names, `.gitignore`, package script names, example file presence only, and redacted manifest metadata. Key names are allowed only when obtained from compatible redacted CLI output or a known value-free manifest. Never inspect full package script command strings, example-file values, copied secrets, encrypted blobs, hashes, fingerprints, partial values, transformed values, or manifest values unless they come from a compatible local CLI or redacted manifest.
4. If env files exist, ask which file or files the user wants to import. Do not read their contents. If the user says "all files" and multiple `.env*` files exist with potentially overlapping keys, warn about key collisions and ask for separate confirmation per file before proceeding.
5. Recommend a value-free dry run before import.
6. Require local interactive approval before importing values or changing project files.
7. Generate or review value-free outputs only: `.env.example`, `secrets.manifest.json`, warnings, classifications, and alias proposals.
8. Use a local run wrapper for commands that need secret injection only after a compatible CLI is installed.
9. Run leak scans as warning-only unless the user approves cleanup.
10. If exposure is possible, recommend rotation and history cleanup where needed.
...
- If CLI status is unknown: run only a non-secret availability check such as `command -v dev-secrets`. If unavailable, return `Status: needs setup` with no runnable `dev-secrets ...` command.
...
These are pseudo-commands until `command -v dev-secrets` succeeds in the target project. Do not show them to the user as runnable commands when the CLI is missing.
...
## Bootstrap Contract For Implementers

A compatible CLI should live on `PATH` as `dev-secrets` or be exposed through a project-local wrapper documented by the project. Minimum commands:

- `doctor`: prints setup status, backend status, and version compatibility without values.
- `status`: prints value-free file counts, manifest status, and warnings.
- `import <env-file> --plan`: prints a value-free plan only.
- `import <env-file>`: imports values locally after interactive approval.
- `scan`: reports locations, classifications, and warnings without values.
- `run -- <command>`: injects values into the child process environment only.

Safety guarantees:

- stdout and stderr never include values, partial values, hashes, fingerprints, encrypted blobs, passphrase hints, or provider tokens.
- failure is closed: no plaintext fallback when encryption, permissions, or backend setup fails.
- mutations require explicit local approval.
- `doctor` is the smallest optional stub, but even a stub must print only setup status and must not read env-file values.
